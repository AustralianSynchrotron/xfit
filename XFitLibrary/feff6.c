  ...
  for (ipotential = 0; ipotential < potentials; ipotential++) {
    ...
    for (number = 0, current_d = -1, next_d = 0.001;
      ...
      for (current_d = next_d, next_d = MAXDOUBLE, shell = 0;
        ...
        d = floor (distance3 (v [shell0], v [shell]) * 1E6 + 0.5) * 1E-6;

        if (fabs (d - current_d) < 5E-7 && number < 8) {
...
/* We replace the Windows call with a unix fork and exec.
  ...
  if(child_id != 0) waitpid(child_id, &current_status, 0);
      ...
      printf ("Forking to feff6...\n");
      status = execlp("feff6",NULL);
      if(status!=0) {           /* shouldn't be here */
          printf("Executing feff6 failed: status %d\n",status);
          return -1;
      }
  }
  ...
  si.cb = sizeof(si);
  sprintf(win_message,"Executing feff6 in directory %s\n",path);
  debug_out(win_message);
  result = CreateProcess (NULL, "feff6", NULL, NULL, FALSE, NORMAL_PRIORITY_CLASS,
                 NULL, path, &si, &Proc);
  if (result == 0) {
	  debug_out("Error starting feff6:");
	  return -1;
  }
  sprintf(win_message,"Feff6 execution result %ld\n",result);
  debug_out(win_message);
  do {
  ...
  /*********************************************************************}
  {                                                                     }
  {  Create a directory to run FEFF6 in.                                }
  {                                                                     }
  {*********************************************************************/
...
/* JRH comments.  We try to get the directory name from environment variables
  ...
  if(mkdir(dir,S_ISUID|S_ISGID|S_IRUSR|S_IWUSR|S_IXUSR)!=0) { 
     printf("Unable to create directory %s for FEFF6\n",dir);
     return NULL;
     }
...
int execute_FEFF6 (char name [],
  ...
  if (feff_inp == NULL) {
    ...
    cwarn ("execute_FEFF6 (%s): can't open %s\\feff.inp", name, path);
  ...
  if (p == NULL) {
    ...
    cwarn ("execute_FEFF6 (%s): %s\\feff.inp is missing the CONTROL card", name, path);
...
int read_FEFF6_path (char wpath [], FILE * paths_dat, path_l * path) {
  ...
  for (leg = 1; leg <= path->legs; leg++) {
    ...
    if (path->potential [l] < 0) {
      ...
      cwarn ("read_FEFF6_path (%s\\path.dat): error reading path %d", wpath, path->number);
...
int read_FEFF6_paths (char wpath [], path_l ** path_list, int read_data,
  ...
  while (read_FEFF6_path (wpath, paths_dat, &path) > 0) {
...
int write_FEFF6_path (char wpath [], FILE * paths_dat, path_l * path) {
...
int save_FEFF6_files (char name [], char path [], int phase, int index) {
  ...
  if (phase)
    ...
    if (pcopy (path, "phase.bin", phases) != 0)
      ...
      cerror ("save_FEFF6_files (%s): can't copy %s\\phase.bin -> %s\\%s",
...
int manage_FEFF6_files (char name [], char path [], int calc, int index) {
  ...
  if (Current_Phase >= 0 && index != Current_Phase) {
    ...
    if (prename (path, "phase.bin", c) != 0)
      ...
      cerror ("manage_FEFF6_files (%s): can't rename %s\\phase.bin -> %s\\%s",
                                              name, path, path, c);
  ...
  if (!calc && Current_Phase != index) {
    ...
    if (prename (path, c, "phase.bin") != 0)
      ...
      cerror ("manage_FEFF6_files (%s): can't rename %s\\%s -> %s\\phase.bin",
                                                       name, path, path, c);
...
int rewrite_FEFF6_paths (char name [], char wpath [], path_l * path, int all,
  ...
  *count = 0;
  ...
  if (paths_dat == NULL) {
    ...
    cwarn ("rewrite_FEFF6_paths (%s): can't open %s\\paths.dat", name, wpath);
  ...
  for (; path; path = path->next) {
    ...
    OK = write_FEFF6_path (wpath, paths_dat, path);
