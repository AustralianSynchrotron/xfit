/***********************************************************************}
{                                                                       }
{  Multiple-scattering FEFF exafs calculation (FEFF 8.20) for xfit      }
{  Adapted from FEFF_601 by James Hester, May 2003
{                                                                       }
{***********************************************************************/

#if !defined (__FEFF_820_C_C)

#define __FEFF_820_C_C
...
parameter_type FEFF_820_parameter_name [] =
...
void destruct_FEFF820_P (FEFF820_P *P) {
...
FEFF820_P * find_FEFF820_P (FEFF820_P P [], int entries, double key) {
...
FEFF820_P * check_FEFF820_P (FEFF820_P * S,
...
double cFEFF_820_EXAFS_i (double parameter [],
  ...
  static FEFF820_P   SP [MAXSAVED] = { 0 };
  ...
  if (first && 0) {
    ...
    clean_FEFF8 ("cFEFF_820_EXAFS_i", working_path);
  ...
  if (status < 0) {
    ...
    for (i = 0; i < MAXSAVED; i++)
      ...
      destruct_FEFF820_P (SP + i);

    manage_FEFF8_files ("cFEFF_820_EXAFS_i", working_path, 0, -1);
  ...
  S = find_FEFF820_P (SP, MAXSAVED, index + 1);
  ...
  */
  ...
  if (S) {
    ...
    match = check_FEFF820_P (S, parameter, polar, normal, ellipticity,
                                                    A, ZA, hole, Nx, k);
...
*/
  ...
  rationalise_FEFF8_potentials ("cFEFF_820_EXAFS_i", ZA, instance,
                                               shells, potential, Z);
  ...
  if (C_position || update_paths) {
    ...
    manage_FEFF8_files ("cFEFF_820_EXAFS_i", working_path, scratch, index);
    ...
    if (update_paths) {
      ...
      execute_FEFF8 ("cFEFF_820_EXAFS_i", working_path, scratch, 1, 0, 0, 0);
      ...
      read_FEFF8_paths (working_path, &path_list, 0, shells, potential, Z, v);
    ...
    if (update_paths)
      ...
      rewrite_FEFF8_paths ("cFEFF_820_EXAFS_i", working_path, path_list, 1, v, &count);

    else

      rewrite_FEFF8_paths ("cFEFF_820_EXAFS_i", working_path, O_path_list, 0, v, &count);
    ...
    if (count) {
      ...
      execute_FEFF8 ("cFEFF_820_EXAFS_i", working_path, 0, 0, 1, 1, 0);
      ...
      read_FEFF8_paths (working_path, &path_list, 1, shells, potential, Z, v);
  ...
  if (save || update_paths) {
    ...
    if (S == NULL)
      ...
      S = find_FEFF820_P (SP, MAXSAVED, 0);
...
int cFEFF_820_EXAFS (double parameter [],
  ...
  if (status < 0) {
    ...
    cFEFF_820_EXAFS_i (NULL, 0, 0, NULL, NULL, 0, 0, 0, NULL, NULL, 0, status, 0);
...
int FEFF_820_EXAFS (double parameter [],
  ...
  return cFEFF_820_EXAFS (parameter, ZA, hole, polar, normal,
...
int FEFF_820_dE_dx (double parameter [],
...
int FEFF_820_calculate (vector model,
  ...
  if (status < 0) {
    ...
    FEFF_820_EXAFS (NULL, 0, 0, NULL, NULL, NULL, NULL, 0, NULL, NULL, status);
  ...
  if (!working_path) {
    ...
    working_path = FEFF8_directory ();

    if (!working_path)

      cerror ("Cannot create a directory to execute FEFF8");
  ...
  if (chi)
    ...
    if (!FEFF_820_EXAFS (&v_el (model, 0),
                         ZA, hole,
                         polar, normal, ellipticity, Q,
                         N, k, chi, status)) {

      cwarn ("FEFF_820_calculate : cannot calculate chi");
  ...
  if (dchidv && N > 0)
    ...
    if (!FEFF_820_dE_dx (&v_el (model, 0),
                       ZA, hole,
                       polar, normal, ellipticity, Q,
                       N, k, dchidv, dv)) {

      cwarn ("FEFF_820_calculate : cannot calculate dchi / dvector");
