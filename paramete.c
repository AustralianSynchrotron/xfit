  ...
  // Parameter types and manipulation routines

#if !defined PARAMETER_CPP
...
double create_parameter_vector (parameter_set * set, int O) {
  ...
  set->vector [O] = realloc (set->vector [O], (count + 1) * sizeof (vector));

  set->key [O] = realloc (set->key [O], (count + 1) * sizeof (double));

  if (set->vector [O] == NULL || set->key [O] == NULL) {
  ...
  set->vector [O][count] = null_vector;
...
int delete_parameter_vector_Oi (parameter_set * set, int O, int i) {
  ...
  for (i++; i < count; i++) {
    ...
    set->vector [O][i - 1] = set->vector [O][i];
